...
class SendZCX02(QDialog):
    def __init__(self, username, token, selected_option, selected_ids):
        ...
        self.setWindowTitle("Send ZCX02")
        ...
        self.invalidation_label = QLabel("cancellationReason \nUzasadnienie anulowania")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.token and file_path and password:
                ...
                if response_status_code == 200:
                    QMessageBox.information(self, "Result", 'Send zcx02 successfully!')
                    self.accept()  # 关闭对话框
                else:
                    if response_status_code == 'error':
                        ...
                        QMessageBox.warning(self, "Error", f'Send zcx02 failed, please try again.')
                        return
            else:
                if not self.token:
