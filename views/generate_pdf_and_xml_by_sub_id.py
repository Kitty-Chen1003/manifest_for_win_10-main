...
class GeneratePDFAndXMLBySubID(QDialog):
    def __init__(self, username, token, selected_option, selected_ids):
        ...
        self.setWindowTitle("Save " + selected_option)
    ...
    def on_confirm(self):
        ...
        if self.selected_option == "XML":
            if self.token:
                dialog = SavePDFOrXML(self.username, self.selected_option, selected_ids, self.token)
                ...
                return  # 返回发送界面，保持对话框打开
        elif self.selected_option == "PDF":
            dialog = SavePDFOrXML(self.username, self.selected_option, selected_ids)
            if dialog.exec_() == QDialog.Accepted:
                self.accept()  # 关闭原对话框
            else:
                print("已取消操作，返回到第一个对话框。")
