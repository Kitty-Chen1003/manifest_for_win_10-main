...
class SendUPD(QDialog):
    def __init__(self, username, token, main_id_list):
        super().__init__()
        self.username = username
        self.token = token
        self.main_id_list = main_id_list

        self.setWindowTitle("Input UPD Signature Info")
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                if self.token and file_path and password:
                    data = {
                        ...
                        'type': 'upd',
                        'main_id_list': self.main_id_list
                    }
                    response_status_code = http_client.upload_reply_message(self.token, data, file_path, password,
                                                                            signature_information)
                    if response_status_code == 200:
                        QMessageBox.information(self, "Result", 'Send upd successfully!')
                        self.accept()  # 关闭原对话框
                    else:
                        if response_status_code == 'error':
                            ...
                            QMessageBox.warning(self, "Error", f'Send upd failed, please try again.')
                            return
                else:
                    if not self.token:
