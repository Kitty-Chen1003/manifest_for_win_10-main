...
class SendZC414(QDialog):
    def __init__(self, username, token, selected_option, selected_ids):
        ...
        self.setWindowTitle("Send ZC414")
        ...
        self.invalidation_label = QLabel("invalidationReason \nUzasadnienie wniosku o unieważnienie")
        main_layout.addWidget(self.invalidation_label)

        # 创建输入框，设置为固定大小
        self.input_field = QTextEdit(self)
        self.input_field.setFixedSize(350, 150)  # 固定大小
        self.input_field.setPlaceholderText("Enter up to 1024 characters...")
    ...
    def limit_text_length(self):
        ...
        if len(text) > 1024:
            self.input_field.setPlainText(text[:1024])  # 截取前1024字符
            QMessageBox.warning(self, "Warning", 'Enter up to 1024 characters...')
            # 将光标移动到文本末尾
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.token and file_path and password:
                ...
                if response_status_code == 200:
                    QMessageBox.information(self, "Result", 'Send zc414 successfully!')
                    self.accept()  # 关闭对话框
                else:
                    if response_status_code == 'error':
                        ...
                        QMessageBox.warning(self, "Error", f'Send zc414 failed, please try again.')
                        return
            else:
                if not self.token:
